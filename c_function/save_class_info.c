...
bool save_class_info(const char* class_title, const char* mentor_name, const char* location, const char* class_description,
                     const char* price, const char* mode, const char* capacity, const char* image_path, const char* tag) {
    // 파일을 추가 모드로 열기 
    ...
    fprintf(file, "%s,%s,%s,%s,%s,%s,%s,0,%s,%s\n", class_title, mentor_name, location, class_description, price, mode, capacity, formatted_image_path, tag);
    fclose(file);
    return true;
}
